<script lang="ts">
  ...
  $: phoneticWithAudio = searchData.phonetics.find(
  ...
  async function playAudio() {
    ...
    if (audio?.src !== phoneticWithAudio.audio) {
  ...
  onMount(() => {
    audio?.addEventListener('playing', setIsPlayingTrue)
    audio?.addEventListener('ended', setIsPlayingFalse)

    return () => {
      audio?.removeEventListener('playing', setIsPlayingTrue)
      audio?.removeEventListener('ended', setIsPlayingFalse)
    }
  })
</script>
    ...
    {#if phoneticWithAudio?.audio}
      ...
      >
        <svg viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
          <circle
            class="fill-purple opacity-25 transition-all group-hover:opacity-100"
            cx="24"
            cy="24"
            r="24"
          />
          <path
            class="fill-purple transition-all group-hover:fill-white"
            fill-rule="evenodd"
            clip-rule="evenodd"
        ...
        {#each meaning.definitions as definition}
          ...
          >
            ...
            {#if definition.example}
              <q class="mt-3 block text-dark-grey">
